...
# Replace string
FILE2=$(<"$file") && \
FILE1=$(<"${_dir}/parse-options-3-rsync.txt") && \
echo "${FILE2//source \$(dirname \$0)\/parse-options-3-rsync.txt/$FILE1}" > "$file"
...
sed -i '/source \$(dirname \$0)\/parse-options-3-rsync-debug.txt/d' "$file"
